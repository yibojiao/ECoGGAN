...
def reading_data(conf_data):
	...
	if conf_data['GAN_model']['seq'] == 0:
		...
		data = np.load(conf_data['data_path'])
		dataset = np.transpose(np.float32(data['arr_0']), (0,1,2,3))[:1000]
		dataset = np.squeeze(dataset, axis = 3)
		if conf_data['GAN_model']['data_label'] == 1:
			...
			dataset_img = dataset
		transform=transforms.Compose([transforms.Resize(g_input_shape), transforms.ToTensor()])
